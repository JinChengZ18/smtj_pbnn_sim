...
> 当前机器上以上工具**均未安装** (已核查，只有 Python)。下面的"可做"指安装后可做；本次已先把不依赖 EDA 的部分 (Verilog-A 模型、Python 金标准、回归脚手架) 做好。
...
|---|---|---|
| **P1 器件 .va + 46 点回归** | ✅ **完全可做** | `.va` 暴露 Sigmoid 观测 → DC 扫描对照金标准 (R²≥0.99)。随机性在 harness。本次 Python 金标准已 PASS (R²=0.9919)。|
...
**结论**：**整条计划基本都能用开源 ngspice 完成**，两点折中——(i) PEX 精度弱于商用；(ii) AMS 共仿改为 Python-in-the-loop。唯一真门槛是**安装**。
...
- **装好 ngspice+OpenVAF 后 (一条命令)**：`python eda/testbenches/run_regression.py` → 编译 `.va`、跑 ngspice、对金标准断言 R²≥0.99。脚本已能在工具缺失时打印安装指引并优雅退出。

## 按顺序的下一步

1. 安装 ngspice≥43 + OpenVAF-Reloaded (见 SETUP)；跑通 P1 回归 (run_regression.py PASS)。 ← **当前卡点：装工具**
