...
- **当前阶段**：**P1（keystone）完成；进入 P2 写路径**。
- **已完成**：`.va` 模型 + Python 金标准 + 随机写 harness + ngspice/OpenVAF OSDI 回归（均验证 PASS）。
- **工具**：ngspice 46 + OpenVAF-Reloaded `20260616-2-gc592eed6` 已安装到当前用户 PATH。
- **下一步**：P2 写路径（task #3）。
...
| ngspice DC 扫描 V(psw) vs 金标准 | `run_regression.py` | 86 点；max\|err\|=3.51e-4；R²=1.00000 | ✅ |
...
| **P1** | `.va` 写好 ✅ + Python 金标准 PASS ✅ + 随机写 harness PASS ✅ + **ngspice `run_regression` PASS** ✅ | ✅ |
