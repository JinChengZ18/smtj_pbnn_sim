"""06a -- Sweep T_full_stack vs. Fashion-MNIST test accuracy for the PBNN-CNN.

Companion to experiment 06 (MNIST + MLP). After training a model with
``05a_fashion_mnist_pbnn_cnn.py``, this script reloads the checkpoint
and evaluates the network at T = 1, 2, 4, ..., 64 in FULL_STACK mode.
It plots accuracy vs T and accuracy vs total per-inference energy (per
the 28 nm PPA tech parameters of the simulator), demonstrating that the
time-domain unfolding trade-off carries over from MLPs on MNIST to a
binary CNN on a substantially harder grayscale image task.

Outputs:
  - runs/06a_fashion_mnist_sweep_T_<ts>/results.csv
  - runs/06a_fashion_mnist_sweep_T_<ts>/06a_fashion_mnist_sweep_T.png
  - figures/06a_fashion_mnist_sweep_T.png

Run from the repo root (after experiment 05a has produced
``runs/fashion_mnist_pbnn_cnn/best.pt``):

    python experiments/06a_fashion_mnist_sweep_T_vs_accuracy.py
"""
...
def _make_pbnn_cnn(*, base_ch: int, hidden_fc: int, num_classes: int,
    ...
    # with sign(theta), so FULL_STACK evaluation should use nominal
    # device parameters to avoid corrupting Bernoulli probabilities
    # with the delta-mode systematic offset (see exp 06 comment).
...
def main() -> None:
    ...
    ckpt_path = REPO / "runs" / "fashion_mnist_pbnn_cnn" / "best.pt"
    if not ckpt_path.exists():
        print(f"Checkpoint not found at {ckpt_path}.")
        print("Run experiments/05a_fashion_mnist_pbnn_cnn.py first.")
        sys.exit(1)

    run_dir = make_run_dir(
        "06a_fashion_mnist_sweep_T", base=REPO / "runs")
    ...
    train_loader, test_loader = get_fashion_mnist_loaders(
        root="./data/fashion_mnist", batch_size=batch_size, num_workers=0)

    Ts = [1, 2, 4, 8, 16, 32, 64]
    accs, energies = [], []
    tech = default_28nm()

    # PPA estimate per inference: sum across all conv + FC layers.
    H, W = 28, 28
    c1, c2 = base_ch, 2 * base_ch
    layer_dims = [
        # (out_features, in_features_per_position, n_spatial_positions)
        (c1, 1 * 3 * 3,                H * W),                 # conv1 28x28
        (c1, c1 * 3 * 3,               H * W),                 # conv2 28x28
        (c2, c1 * 3 * 3,               (H // 2) * (W // 2)),    # conv3 14x14
        (c2, c2 * 3 * 3,               (H // 2) * (W // 2)),    # conv4 14x14
        (hidden_fc,   c2 * 7 * 7,      1),                     # fc1
    ...
    axes[0].set_title("Fashion-MNIST PBNN-CNN: accuracy vs. T")
    ...
    axes[1].set_title("Fashion-MNIST PBNN-CNN: accuracy vs. energy")
    axes[1].grid(alpha=0.3, which="both")

    fig.tight_layout()
    out = REPO / "figures" / "06a_fashion_mnist_sweep_T.png"
    fig.savefig(out, dpi=180, bbox_inches="tight")
    shutil.copy2(out, run_dir / "06a_fashion_mnist_sweep_T.png")
