...
- [◑] **B1 = T1-1**：免复位马尔可夫采样。**前置检查完成（2026-07-08）——双绿灯，全量投入**：(1) 中段质量充足：fc1/fc2 的 p∈[0.2,0.8] 占 14.7%/13.7%（约 26 万单元）、fc3 占 32.1%，饱和(<1%或>99%)仅 ~57%——θ×100 退化顾虑排除；(2) 双极性标定可用：`data/smtj_psw_curves/measured_0p75ns.csv` 含 A/B 两器件 × AP→P/P→AP 两方向（16+12+9+9=46 点），`calibration.py:113 fit_per_device_direction` 现成——读后择极性校正可从入库数据标定。**余步**：有状态免复位采样路径（复用 telegraph.py 条件传播形式，穿透 unfold.py + pbnn_linear/pbnn_conv 两处 FULL_STACK）→ 马尔可夫偏差-T 曲线与精度冲击 → 读后择极性/去偏两校正 + 8 种子 D2D 稳健性 → 复位相能量入 PPA、三时序前沿 → 第四章能耗口径与 psw_cond 脚注闭环（稿件段等落定）。
