...
## 当前状态  （last update: 2026-07-08；上一次快照 2026-07-06，详录 2026-06-28）

- **📌 2026-07-08 快照（MTJ L1 完成 + DRC 假阴性勘误）**：(1) **2T SOT-sMTJ 单元版图落地**（`eda/hero/layout/gen_2t_cell.py`→`cell2t.gds`）：全脚本布线（无 GUI）、CMOS 部分 **DRC 真 0 违例**（feol+beol+offgrid 开关 + 阳性对照验证）、Magic PEX 提出 2 FET（w=2.2/0.42）+ ~3.1 fF、设计层 bbox 22.7 µm²、黑盒层 200/0+201/0（层空位双重核查）、剖面图 `figures/cell2t_cross_section.*`；串扰证书以实绘 pitch 4.06 µm 回灌（δp=1.4e-6）。(2) **⚠️ DRC 假阴性勘误**：`sky130A_mr.drc` 不传 `$feol/$beol/$offgrid` 开关时不执行任何规则、恒报 0——2026-06-26 的「SA 器件级 DRC 0 违例」系假阴性，带开关实得 ~542 条（OFFGRID 为主 + m1.5/li.3，均为 PCell 偏格/压线伪影而非设计错误；`gen_2t_cell.py` 的全层 5-dbu 吸附即修复方案，SA 修复随 1.7 布线窗口）。正文 ch4 未宣称 DRC-clean，勘误范围限内部记录（已改：run_drc.sh、layout/README、PLAN_execution 0.3/1.5 行、本文件）。**今后任何 deck 的 0 违例结论必须先过阳性对照。**

- **📌 2026-07-06 快照（2026-06-29→07-06 期间进展的汇总，细节见 [`../MEMORY.md`](../MEMORY.md) 对应日期条目）**：(1) P3/P4 设计对比程序**端到端收口**——附录 D 终态 = 11 图 + 7 表、11 个同流程实测设计 + 引用性定性行，所有定量数可由入库脚本再生（SAR 数字已修正：monotonic 92.3 fJ、节省 4.3×，勿用旧值 457.4/140.6/841/525）；(2) 比较器矩阵 5 设计（dong_autozero σ/V_T=0.167 = 失调消除可复现，plain StrongARM 名义点仍 Pareto 最优）；(3) 全部旁支已并入 master（θ_SH=0.066 重标定传播 + LLG 交叉验证 0.40·V_T 口径统一）；(4) VA 同步 PARKED（等 canonical vgsot-sim 验证，见 `VA_SYNC_RUNBOOK.md`）；(5) 引文核查 2026-07-03（4 处更正，见 `../reference_fact_check_2026-07-03.md`）；(6) errata 回填检查表 2026-07-06 逐项核验勾销（R5 留口径注记）。**当前开口项清单以 [`PLAN_execution.md`](PLAN_execution.md) 末节为准；新方向统筹见 [`../plans/`](../plans/)。**
...
- **已完成**：
  ...
  - **Hero(A1)：sky130 StrongARM SA**（`hero/strongarm_sa.spice`，vind=+20mV→outp=1.8V）→ **输入折合失调 σ=9.21mV=0.39·V_T**（`run_offset_mc.py` N=120 firmed；N=24 早值 11.05 偏高；关 R2 的核心可信数）→ **闭环 MNIST**（per-column σ=8→96.35%）→ **版图导出 GDS**（`hero/layout/`，sky130 PCell 9 器件，611 shapes）。Magic/TCL 被版本卡（需 Magic≥8.3.306），改用 KLayout PCell 流。
...
| **Hero(A1) SA 输入折合失调 MC（sky130 真跑）** | `eda/hero/run_offset_mc.py`（WSL ngspice+sky130） | StrongARM σ_offset=**9.21mV=0.39·V_T**，3σ=1.18·V_T（**N=120 firmed**，SE≈0.6mV；N=24 早值 11.05/0.47 偏高，AVT 假设） | ✅ |
...
| **Hero(A1) 版图导出 → GDS（"导出版图"交付）** | `eda/hero/layout/gen_sa_layout.py`（WSL KLayout sky130 PCells） | StrongARM 9 器件（5 NMOS+4 PMOS，带保护环）→ `sa_devices.gds`：top `strongarm_sa_devs`，17.5×18.7µm，**611 shapes，sky130 真层号**（diff 65/20, poly 66/20, li1 67/20, met1 68/20…）。Magic/TCL 路线被版本卡（Magic 8.3.105 < 需 8.3.306）→改 KLayout PCell 流。~~DRC 已通过：0 violations~~（⚠️ 2026-07-08 勘误：该 DRC 结论为无开关假阴性，带开关实为 ~542 条 PCell 偏格/压线伪影，见 eda/hero/layout/README.md「DRC 特性开关」；提取数不受影响）（`run_drc.sh`，经 ASCII build dir `/home/lenovo/smtj_eda_build` 跑 sky130A_mr.drc；器件级，布线 DRC 待加互连）。WSL 链坑（~~CJK 路径破坏 `-rd input` UTF-8 解析~~【迁英文路径后已不适用】 / `/tmp` 空闲清空 / `bash -lc` 内变量丢失）已由 build-dir 方案规避 | ✅ 器件版图 + DRC 0-violation |
| **Hero(A1) B5 读出映射：mV→popcount→精度（闭环合拢）** | `eda/hero/readout_mapping.py`（纯 Python，吃上游 JSON） | 跨阻 `LSB_V=LSB_I·R_TI` 桥接 P3 的 5.1µA/pc、SA 的 9.21mV(N=120)、per-column 精度曲线；协同律 `σ_pc=σ_offset_V·2·PC_FS/V_in`。**最大增益读出下 plain SA 0.39·V_T → σ_pc≈2–4 → 精度跌<0.15pp**（R_TI≈400–700Ω）；仅 V_in=0.4V+宽扇入越膝点。结论=**量化设计边界**（何时省/需自调零），非「必须自调零」 | ✅ first-cut（闭环 mV→精度合拢）|
...
| **Track A SA 版后（器件集修正 + 寄生 + LVS 工具链）** | `gen_sa_layout.py`(11器件) + `run_pex.sh` + `sa_postlayout.py` + netgen | 版图器件集 9→**11**（补 Mp3/Mp4，匹配原理图），~~DRC 0 违例~~（⚠️ 2026-07-08 勘误：无开关假阴性，实为 PCell 偏格/压线伪影 ~542 条，见 2026-07-08 快照；提取数不受影响），提取 **11 MOSFET + 35.25 fF 器件 C**；SA 动态能 ~**23–74 fJ/决策**（5–15× 5fF 占位→R1 读出低估）；失调对称布线设计律→R2。netgen 1.5.321 LVS 工具链打通（设备级；完整 LVS 待布线，见 `layout/LVS_GUI_CHECKLIST.md`） | ◑ first-cut（布线/全 LVS 待 GUI 收尾）|
| **1.11 C1 失调消除 Pareto（accuracy vs V_offset/V_T）** | `eda/hero/pareto_offset_cancellation.py`（纯 Python，吃 hero_mnist_summary + 协同律）| {无/4×面积/单容自调零/两相斩波}×读出工作点；噪声地板 0.15pp。**V_in≥0.5V/MNIST 扇入 → plain SA Pareto 最优**（差落噪声内）；**仅 V_in≤0.4V/宽扇入 → 自调零挣回成本**（边界 layer1≤0.29V、layer2≤0.34V，σ firmed 9.21mV 后更宽）。规格反转=按 V_T 预算失调、非 TMR 余量 | ✅（关 R2 设计边界）|
...
| **B1-val 双模型：LLG（vgsot-sim）验证行为级 sigmoid（指令②）** | `eda/testbenches/llg_validate.py`（驱动 vgsot-sim MC P_sw vs I_SOT，自热ON，200 trials/点，映射 V=I·776）| **LLG 物理独立复现标定行为级 sigmoid（FL-SOT 勘误 + θ_SH=0.066 重标定后，2026-06-30 更新）**：阈值 **LLG 0.886V vs 行为级 V_th 0.896V → 差 ~9mV=0.40·V_T（PASS，200 trials）**；但勘误后 LLG 曲线比实测 sigmoid **明显更宽**（RMSE=0.16、对单调 sigmoid 的 R²<0，此前 bug 下为 R²=0.92）——单宏自旋与 NB 一致地**低估 C2C 收窄后的实测斜率（η_c 缺口）**，FL-SOT 符号 bug 曾"人为锐化"曲线掩盖之。两模型**阈值吻合**，斜率差异属预期亚畴效应、非标定误差；高压端进一步偏离=过驱进动回切。**双模型策略落地**：行为级主力迭代、LLG 验证 | ✅（验证 B1）|
...
| **C3/F3 列共享 SAR 读出：cap-DAC 能量坐实（sky130）** | `eda/testbenches/sar_capdac_energy.py`（纯 Python，sky130 电容密度+SAR 开关能量公式） | 比较器=提取 StrongARM 48fJ；cap-DAC 由 C_u≈1.5fF 按常规/单调两方案给上下界，rc_isoenergy 假设 E_capDAC0=1.1fJ·2^b **落在二者之间**（可辩护）；b=8 阵列 51–509fJ 与比较器 384fJ 同量级 → 比较器主导、b-线性 → **列共享时分复用摊销**为能量杠杆（非降分辨）。坐实 F3 能量 | ✅（F3 能量坐实）|
| **C2/F1 写-DAC 电路拓扑选型（sky130）** | `eda/hero/run_write_dac.py`（WSL ngspice 码扫描） | 一阶电流舵 DAC 接 776Ω 低阻写负载→随 V_load 升电流源失裕、**非单调**（INL≈1.7 LSB）、量程偏窄；简单共源共栅枯竭 → **改电压型电阻串**（构造单调，LSB=Vspan/2^b；200mV 量程 6–7bit→LSB 1.6–3.1mV<V_T/7，覆盖 148mV 预畸变+微调），抽头经写驱动缓冲入写线。承载 C2 trim + F1 预畸变码 | ✅（拓扑选型，仿真定） |
| **C1 读出前端电路：跨阻+StrongARM 链路（sky130）** | `eda/hero/run_readout_frontend.py`（WSL ngspice+sky130，popcount 电流扫描） | 斜率匹配协同律落到电路：R_TI=V_in/(2·PC_FS·LSB_I)=**613Ω**（满量程用满半输入范围）+ StrongARM；提取失调（9.21mV）在整条链路映射 **σ_pc≈2.5 popcount**（≈理想律 2.95 的 85%，差额=SA 有限输入阻抗负载）<膝点 → plain SA 电路级足够。把协同律从解析坐实为可仿真读出电路 | ✅（C1 电路闭合）|
