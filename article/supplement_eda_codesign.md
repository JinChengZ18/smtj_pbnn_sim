...
为给后续逐模块的协同设计提供系统语境，图9先给出由这些模块集成的单条计算通路（写—存—读）的晶体管级原理图：电压型电阻串写数模转换器经 CMOS 写驱动把写电压送上写线 BL，2T SOT-MTJ 单元在写字线 WWL 选通下被写入、源线 SL 为公共回流；读出时读字线 RWL 选通，列读线 RBL 上的电流经跨阻 $$R_\mathrm{TI}$$ 转为电压，由 StrongARM 比较器作概率位二值判决，同一列节点亦被列共享逐次逼近转换器在储备池模式下分时读出。其后各节依次坐实其中各模块的器件级设计；阵列沿行共享 BL/SL/RBL、沿列共享字线与列共享转换器。

![图9 集成计算通路（sky130，由 Xschem 导出）](figs/Supplement_local_09.png)

**图9** sMTJ 概率位／储备池计算通路的晶体管级集成原理图。写通路（左）：写数模转换器 $$\to$$ CMOS 写驱动 $$\to$$ 写线 BL $$\to$$ 写访问管（WWL 选通）$$\to$$ 2T SOT-MTJ 单元 $$\to$$ 源线 SL。读通路（右）：读访问管（RWL 选通）$$\to$$ 列读线 RBL $$\to$$ 跨阻 $$R_\mathrm{TI}$$ $$\to$$ StrongARM 比较器（概率位读出，输出 p_out）；同一列节点经列共享逐次逼近转换器在储备池模式下输出多位码，两种读出模式在时间上互斥。存储单元为器件级绘制，写数模转换器与逐次逼近寄存器等子模块以含真实端口网络的功能块表示，其器件级原理图分见图6—图8。

图9 给出单条通路的器件级实现，图10 则给出由器件到系统的整体架构层次。自左向右依次为：SOT-MTJ 随机器件（重金属轨上的自由层／隧穿势垒／钉扎层叠层，热涨落驱动开关概率 Sigmoid，兼作熵源与计算单元）、2T SOT-MTJ XNOR 存内计算阵列（基尔霍夫电流定律完成列 popcount）、sky130 CMOS 外围（写通路、读通路、行列译码、模式与时序控制，经 BL／SL、WWL／RWL、RBL 与阵列耦合）、以及概率位与储备池两种分时复用的工作模式；顶部虚线为闭环采样反馈。底栏标出各层的抽象级别与其落地来源——器件层为晶圆标定紧凑模型（LLG 交叉验证）、阵列层为行为级 XNOR-CIM、外围层为 sky130 提取数（StrongARM 读出、写线 IR、SAR 电容阵列）、系统层为 PPA 能量模型。

![图10 由器件到系统的整体架构层次（sky130）](figs/Supplement_local_10.png)

**图10** sMTJ 概率位／储备池计算系统架构（器件 → XNOR-CIM 阵列 → sky130 CMOS 外围 → 系统）。SOT-MTJ 随机器件经 2T XNOR 存内计算阵列、sky130 CMOS 外围（写／读通路、行列译码、模式时序控制器，经 BL/SL · WWL/RWL · RBL 与阵列耦合）、至概率位与储备池两种分时复用模式；顶部虚线为闭环采样反馈 $$x^{(r+1)}$$。底栏给出各层抽象与 sky130 落地的对应，其中外围层（读出、写线 IR、电容阵列）为提取数（红色标注）。
...
在 sky130 上实现 StrongARM 锁存比较器[^strongarm]，对输入对与锁存管阈值失配做 120 次蒙特卡洛（Pelgrom 面积反比律[^pelgrom]，失配系数取该工艺量级），得到输入折合失调 $$\sigma_\mathrm{off}=9.21\,\mathrm{mV}=0.39\,V_T$$。即未经失调消除的平凡比较器，其失调已与判决窗同量级，会以每输出列一个系统性阈值偏移的形式注入误差。
...
**图7** 写通路的晶体管级实现。电压型电阻串写数模转换器（按构造单调，最低有效位 = 参考量程／$$2^b$$；图中以四个匹配单位电阻示意，实际为 $$N$$ 抽头）经 CMOS 传输门[^tgate]完成 $$k{:}1$$ 抽头选择，得到写电压 $$V_\mathrm{wdac}$$，再由一级 CMOS 推挽写驱动缓冲到写行线 WRL；写线串联寄生电阻 $$R_\mathrm{line}$$ 表征 IR 压降，IR 感知逐行预畸变把目标码抬升 $$\mathrm{dD}(I_\mathrm{wr},R_\mathrm{line})$$ 以在远端抵消之。存储单元为 2T SOT-MTJ：访问 NMOS 由写字线 WWL 选通，三端 SOT-MTJ 的写电流经重金属轨从写线流向源线 SL，其读出端 RD 引出至灵敏放大器。器件均为 sky130 130 nm 工艺单元，标注沟道宽长比；原理图连通性已由网表逐一核对。
...
储备池读出的能量则需更细的协同。原储备池能量模型把读出当作近乎免费，而读出精度实为记忆容量的限制者，二者矛盾。为此引入一个三方协同模型（图 4a）：在节点数 $$N$$、被数字化节点数 $$M$$、模数转换位数 $$b$$ 三者之间，对固定能量预算最大化记忆容量，模数转换能量以提取的 StrongARM 比较器作逐次逼近转换器[^sar]的比较器项。结果是：读出确非免费——比较器即使 3 位分辨也已占储备池总能量近九成；但比较器项随位数线性增长，故分辨率的能量惩罚温和。需要说明的是，对集成式储备池而言"低/中等分辨读出即足"这一结论本身在近期工作中已有报道[^ens_rc]；本文区别于该工作之处，在于以**记忆容量每焦耳**为目标、把**节点数 $$N$$ 纳入联合权衡**、并采用**列共享模数转换器**摊销转换能量于超顺磁电报噪声节点之上。由此得到的读出架构是列共享、时分复用的中分辨逐次逼近转换器，而非逐节点高分辨转换。该转换器的能量已坐实于工艺数据：比较器取提取的 StrongARM，电容数模阵列的开关能量由 sky130 单位电容（MIM/MOM，$$C_u\!\approx\!1.5\,\mathrm{fF}$$）按常规与单调两种开关方案给出上下界——所用的每转换电容能量假设落在二者之间（在 8 位下电容阵列约 51–509 fJ，与比较器的 384 fJ 同量级），故能量模型不偏乐观；比较器与电容阵列同量级且比较器项随位数线性，正是"摊销共享比较器（列共享、时分复用）而非降低分辨率"成为能量杠杆的原因。
...
![图8 列共享逐次逼近读出电路（sky130，由 Xschem 导出）](figs/Supplement_local_08.png)

**图8** 列共享、时分复用的逐次逼近读出电路。多列抽头经采样开关（sel0/sel1）时分复用到单一电荷再分配电容数模阵列的共享顶板节点 $$V_x$$：二进制加权电容（图中示意 $$4C{:}2C{:}C$$）顶板并接于 $$V_x$$，底板由各位开关在 VREF 与 GND 之间切换；StrongARM 比较器（三角形符号，其晶体管级网表见图6）将 $$V_x$$ 与共模电平 VCM 比较，逐次逼近寄存器据比较结果驱动各位开关并逐位逼近，输出数字码 Dout。一套比较器与电容数模阵列在各列间摊销、时分复用，正是"摊销共享转换器而非降低分辨率"这一读出能量杠杆的电路实现。
...
前述各模块的功能由瞬态仿真给出端到端佐证（图11）。写通路上，0.9 V、0.75 ns 的写脉冲经 776 Ω SOT 支路交付，端电压在亚纳秒内建立，标定紧凑模型给出的瞬时开关概率 $$P_\mathrm{sw}(t)$$ 在脉冲窗内升起、脉冲撤去后归零——一次写即产生一个伯努利样本（图11a，ngspice）。读出端，StrongARM 锁存在时钟到来前把两输出预充至 $$V_\mathrm{DD}$$，时钟沿后再生正反馈在数百皮秒内把 10 mV 差分输入放大为轨到轨判决（out+ 升、out− 降），判决完成后无直流通路（图11b，ngspice）。储备池读出端，列共享逐次逼近转换器以电荷再分配二分搜索，在 8 个比特试探内把所采样列电压逼近至最低有效位（图11c）。

![图11 三类操作的瞬态波形（sky130 / ngspice）](figs/Supplement_local_11.png)

**图11** 三类操作的瞬态波形。(a) 写脉冲交付与瞬时开关概率 $$P_\mathrm{sw}(t)$$（含标定 smtj_sot 紧凑模型）。(b) StrongARM 再生：时钟沿后由预充态分裂为轨到轨判决（10 mV 差分输入）。(c) 列共享逐次逼近转换器的电荷再分配逐位逼近（8 位）。(a)(b) 为真实 ngspice 瞬态，(c) 为同一列节点上的逐次逼近算法。

两种工作模式在同一阵列上分时复用，其相位流水线见图12。概率位推断模式下，一次判决由 $$T$$ 个伯努利样本构成，每个样本是一个"写（Φ_w：写驱动经 BL／WWL 写入）—弛豫—读（Φ_r：经 RWL／RBL 由 StrongARM 判决）"的相位循环，$$T$$ 次读出取平均近似 $$\mathbb{E}[s]$$，置信度早退可缩短 $$T$$；储备池模式下，先写入输入、再令低势垒器件自由随机演化以产生丰富动力学，最后由列共享逐次逼近转换器分时扫描 $$M$$ 列完成读出。低势垒所需的强循环噪声与概率位的写保持相冲突，故两种模式互斥地分时占用同一物理阵列。

![图12 工作模式流水线与相位时序](figs/Supplement_local_12.png)

**图12** 工作模式流水线。(a) 概率位推断：每样本为 写（Φ_w）—弛豫—读（Φ_r）相位循环，$$T$$ 次平均得 $$\mathbb{E}[s]$$，置信度早退缩短 $$T$$。(b) 储备池处理：写入输入 → 低势垒自由随机演化 → 列共享逐次逼近转换器分时扫描 $$M$$ 列得读出向量。两模式分时复用同一阵列、在时间上互斥。
...
[^strongarm]: StrongARM 锁存比较器是一种时钟触发的动态再生锁存比较器，由差分输入对、交叉耦合再生锁存与预充电管构成：时钟到来时再生正反馈把毫伏级差分输入放大为轨到轨数字判决，判决稳定后无直流通路、静态功耗为零。本文采用它而非静态预放大型比较器，是因为按列复制的概率位／储备池读出要求每列一个常驻判决单元，零静态功耗、轨到轨数字输出与紧凑面积三者对此至关重要；它亦是存储器与存内计算灵敏放大器的主流拓扑。其失调主要来自输入对与锁存管的阈值失配，可由版图面积及（按需启用的）自调零抑制。
[^pelgrom]: Pelgrom 失配律：相邻器件参数（此处为阈值电压）的随机失配标准差与器件面积平方根成反比，$$\sigma_{\Delta V_\mathrm{th}}=A_{V_T}/\sqrt{WL}$$，$$A_{V_T}$$ 为工艺相关失配系数。本文据此由器件面积预算比较器输入折合失调并做蒙特卡洛抽样。
[^tgate]: CMOS 传输门由并联的 NMOS 与 PMOS 构成，对轨到轨范围内的模拟电压提供低且较对称的导通电阻；相较单管开关（近电源轨时损失约一个阈值电压、导通电阻随电平强烈变化），它能近乎无失真地把所选电阻串抽头电压传给写驱动，是模拟选择开关的常规做法。
[^sar]: 逐次逼近（SAR）模数转换器以单个比较器配合电荷再分配电容数模阵列，按二分搜索逐位逼近输入，每次转换仅需一个比较器、无需运算放大器，转换能量随分辨率近似线性。相较闪烁型（器件数随分辨率指数增长）与过采样 Σ-Δ 型（高分辨但吞吐低、需抽取滤波），SAR 在中等分辨率（约 8–10 位）、中等速率区间能效最优，且其单比较器结构天然适合在多列间分时复用——契合储备池读出"中分辨、能量优先、可列共享"的需求。
