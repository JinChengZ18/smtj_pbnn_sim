...
图9 给出单条通路的器件级实现，图10 则在片（tile）层面给出各外围模块与 sMTJ 阵列的耦合接口：阵列以行共享字线、列共享位／源／读线组织；写列驱动（写数模转换器＋IR 预畸变＋CMOS 驱动，按列复制）自顶部经位线 BL／源线 SL 向阵列交付写电压；行译码与字线驱动自左侧经写字线 WWL／读字线 RWL 选择被写或被读的行；列读出自底部经读位线 RBL 感知列电流，由 StrongARM 比较器给出概率位判决、或由列共享逐次逼近转换器给出储备池多位读出；模式与时序控制器统一编排写／读相位、概率位与储备池两种读出模式的分时复用、$$T$$ 次采样平均与列共享选择。本图只刻画模块与阵列的耦合接口，模块内部拓扑见图6—图9。

![图10 计算 tile 的电路架构与模块—阵列耦合（sky130）](figs/Supplement_local_10.png)

**图10** sMTJ 概率位／储备池计算 tile 的电路架构：各外围模块与 2T SOT-MTJ 阵列的耦合接口（不含模块内部电路）。写列驱动经 BL／SL 自顶部耦合、行译码经 WWL／RWL 自左侧耦合、列读出经 RBL／SL 自底部耦合；模式与时序控制器（虚线为控制／时钟相位）统一编排写读相位与两种读出模式的分时复用，一套逐次逼近转换器经读出选择在多列间共享。
