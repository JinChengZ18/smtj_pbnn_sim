...
**图5.9** 储备池读出的能量协同与外围方案的同流程对比。(a) 等能量前沿：记忆容量对每步能量、颜色为模数转换位数；读出主导能量 (比较器即使低分辨也占近九成)，但比较器项随位数线性增长，故分辨率的能量惩罚温和——支撑以列共享摊销比较器、而非降低分辨率为读出能量杠杆。(b) 计入物理模数转换读出后，sMTJ-RC相对数字ESN的能量优势 (逐节点／列共享、不同位数)，落在约30–35×。(c) 两种SAR电容数模阵列开关方案 (本文monotonic与conventional) 在同一sky130暂态电荷积分下的单次转换能量 ($$b=8$$，电容DAC开关能与已提取比较器能分项堆叠)：monotonic把电容DAC开关能压低约4.3倍 (92.3对394.7 fJ)，而按位线性的比较器项主导总能量，印证读出能量杠杆在于跨列摊销比较器而非削减位数。各备选方案与同流程分析见附录D。
