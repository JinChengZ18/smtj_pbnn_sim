...
上述温轴采用固定势垒高度的简化，即$$\Delta(T)=\Delta_{300}\cdot 300/T$$只随分母kT缩放；把第四章建立的磁性标度链引入后，势垒本身也随温度下降 (界面项与形状项的近补偿之差被温度放大)，修正后的$$\tau(T)$$在高温端明显更短，400 K处约为简化口径的一半[^tau_axis_note]。热时钟配方的形式不受影响：系统时钟跟随$$\tau(T)$$即可，与其取何口径无关；但按温反推势垒或设定重标定周期时应使用修正温轴。修正轴与第四章概率二值网络的部署温窗共享同一标度链，两种工作模式的温度行为由此统一在同一温轴上 (第四章图4.24)。

[^tau_axis_note]: 本节温度轴经历一次口径订正：初版温度研究即采用仅kT缩放的简化轴，第四章温度链完成后经交叉核对，确认高温端弛豫时间被简化口径高估约2倍；图5.4的定性结论 (强敏感性与时钟补偿有效性) 不受影响，定量温轴以修正口径为准。
