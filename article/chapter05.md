...
**图5.9** 储备池读出的能量协同与设计空间。(a) 等能量前沿：记忆容量对每步能量、颜色为模数转换位数；读出主导能量 (比较器即使低分辨也占近九成)，但比较器项随位数线性增长，故分辨率的能量惩罚温和——支撑以列共享摊销比较器、而非降低分辨率为读出能量杠杆。(b) 计入物理模数转换读出后，sMTJ-RC相对数字ESN的能量优势 (逐节点／列共享、不同位数)，落在约30–35×。(c) 列共享逐次逼近读出与近期文献多方案对比：灵敏放大失调比$$V_T$$对比较器能量 (对数)，本文唯一同时给出已提取的比较器能量与按$$V_T$$预算的失调 (数据来自多智能体arXiv/开放获取调研)。
