...
Mackey-Glass单步预测NRMSE低至0.006，随预测步长增至约0.04，表明储备池能稳定承载混沌动力学的短时预测。容量分解 (图5.6(c)，以各阶延迟单项式的保留$$r^2$$之和度量，作为信息处理容量的实用代理；因所用单项式非正交，各阶之和不构成受储备池维数约束的绝对IPC，仅用于均值场与器件、以及偏置扫描间的相对比较) 给出一个与5.4节自洽的结构性结论：零偏工作点下线性容量与三次容量较大，二次容量接近零，这正是$$\tanh$$奇非线性的反映，奇阶项可由器件直接生成，偶阶项需偏置打破对称才会出现。图5.6(d) 显示，随偏置升高，二次容量逐步释放，线性容量相应让渡，总容量大致守恒。这把5.4节的记忆与非线性权衡从单一乘积任务推广到完整的多项式容量谱，说明工作点偏置是在固定器件上重新分配记忆与非线性预算的连续参数。
...
在$$N=100$$、1000步推理下，sMTJ-RC约$$338\,\mathrm{nJ}$$，其读出能量以提取的sky130 StrongARM比较器与电容数模阵列标定的逐次逼近转换为准、不再当作免费 (外围电路的工艺节点与各项提取方式统一见4.6节：开源sky130 130 nm，比较器判决能由Magic提取的器件电容结合ngspice瞬态积分得到，电容数模阵列开关能以sky130单位电容密度标定)；较常规数字ESN (约$$10\,204\,\mathrm{nJ}$$，按1 pJ每次乘加计) 低约30倍 (逐节点8位读出；改用列共享时分复用读出约35倍)，按单位记忆容量能耗计低约一个数量级 (231对2140 nJ)。又因$$O(N)$$对$$O(N^2)$$的标度分野，该优势随储备池规模增大而扩大 (图5.7(c))。需要诚实指出的是，理想存内计算ESN (忽略ADC开销) 下界仍低于sMTJ-RC，故该优势是相对冯诺依曼式数字ESN而言，而非相对最激进的模拟存内计算。图5.7(d) 也显示sMTJ-RC以更低能耗占据较低精度区间，要达到数字ESN的记忆容量需更大集成，二者在精度与能量平面上各有占优区。综合而言，sMTJ-RC的硬件价值在于以器件物理承担最昂贵的稠密递归，在能量预算受限、精度要求中等的时序处理场景具备明确优势。

上述能量对比把读出连同模数转换计入，其底层是一处值得单独说明的协同设计。原储备池能量模型把读出当作近乎免费，而读出精度实为记忆容量的限制者，二者矛盾。把模数转换能量以提取的sky130 StrongARM比较器作逐次逼近转换器的比较器项、电容数模阵列的开关能量以sky130单位电容标定后，读出确非免费：比较器即使3位分辨也已占储备池总能量近九成 (图5.9)；但比较器项随位数线性增长，故分辨率的能量惩罚温和。对集成式储备池而言低至中等分辨读出即足这一结论本身在近期工作中已有报道[^ens_rc]，本章区别于该工作之处在于以记忆容量每焦耳为目标、把节点数纳入联合权衡，并采用列共享、时分复用的逐次逼近转换器，把一套比较器与电容阵列摊销到超顺磁电报噪声节点之上 (图5.8)。正是这一摊销使前述能量优势在含提取读出时仍达约30倍 (逐节点8位) 至约35倍 (列共享)，而非以降低分辨率换取。
...
**图5.9** 储备池读出的能量协同与文献能力对照。(a) 等能量前沿：记忆容量对每步能量、颜色为模数转换位数；读出主导能量 (比较器即使低分辨也占近九成)，但比较器项随位数线性增长，故分辨率的能量惩罚温和——支撑以列共享摊销比较器、而非降低分辨率为读出能量杠杆。(b) 计入物理模数转换读出后，sMTJ-RC相对数字ESN的能量优势 (逐节点／列共享、不同位数)，落在约30–35×。(c) 列共享逐次逼近读出与近期文献多方案的能力矩阵：逐项标注各设计是否给出已提取的比较器能量、按$$V_T$$窗预算的失调、列共享摊销、纯电荷再分配SAR、比较器/电容DAC分项及RC/CIM场景，本文为唯一同时具备者 (定性对照，不含外推数值；各方案引文见submodule_survey数据集)。

## 5.6 本章小结

本章把全文主线下同一硬件单元在时序处理任务上的可达性这一问题，落到第四章仿真器之上的两个新增层，即有状态电报模型层与储备池读出层，并以噪声无关的均值场模式作为理想参照贯穿评估。主要结论有四点。第一，可计算性方面，sMTJ以随机电报动力学运行时，在记忆容量与混沌预测等标准基准上具备实在的计算能力，记忆来源于磁矩弛豫时间这一可由电压指数调控的物理量。第二，器件优化方面，面向RC的sMTJ应比PBNN写器件势垒更低 (本工作工作点下最优$$\Delta$$约为4，平缓高原覆盖3.5至4.3，对$$4.91$$)，并按弛豫时间约为采样周期二至三倍的关系选择势垒，温度对$$\tau$$的强敏感性可由跟随$$\tau(T)$$的系统时钟予以补偿。第三，鲁棒性方面，对PBNN有害的D2D变异对RC从无害转为有益，器件失配被转化为节点多样性，真正的限制是读出信噪比。第四，硬件能效方面，sMTJ储备池以器件物理替代数字ESN的$$O(N^2)$$稠密递归，相对常规数字ESN能效高约一个数量级，且优势随规模扩大。
...
把同一物理阵列以时分复用同时承担概率位推断与储备池处理，还要求两种模式取不同的热稳定因子：概率位取$$\Delta=4.91$$ (零偏弛豫约68 ns)，储备池取$$\Delta\approx3.8$$ (约22 ns)。一块固定器件不能同时是两个势垒，时分复用需约22.6%的势垒摆幅 ($$\Delta E_b\approx28.7\,\mathrm{meV}$$)，可经电压控制磁各向异性栅 (约0.56 V，量级与已报道的电压可调各向异性器件相当) 或约88 K温升达到。但三点限制必须登记：两种模式在时间上互斥；储备池所需的低势垒与概率位的写保持及读扰动相冲突；尚无在同一物理阵列上时分复用概率位写入与储备池自由演化的实现，已报道的电压可调随机性器件或为脉冲驱动的可调随机电报噪声[^tunable_rtn]，或为电压控制各向异性的确定与泊松双功能宏[^vcma_macro]，均非概率位与储备池双模阵列。因此该统一阵列应作为受限的、电压栅控的、时分复用的架构提案陈述，并附上述量化调谐包络与势垒冲突这一限制。
...
**图5.10** sMTJ概率位／储备池计算的整体系统架构 (器件 → XNOR存内计算阵列 → sky130 CMOS外围 → 操作模式)。同一物理阵列经BL/SL、WWL/RWL、RBL与外围耦合，右侧操作模式层并列概率位推断与储备池两种时分复用用法，顶部虚线为闭环采样反馈，底栏标出各层的抽象级别及其sky130落地来源 (外围层为提取数)。
...
[^ens_rc]: Ensemble reservoir computing for physical systems. arXiv:2601.21807, 2026（扫描集成数与模数转换位数，指出低至中等分辨即足）。
...
[^vcma_macro]: 一种28 nm CMOS集成的VCMA-MTJ双功能宏，用于确定性存内计算突触与随机泊松神经元。*IEEE Symposium on VLSI Technology & Circuits*, 2026.
