...
固定时钟下记忆容量在250至410 K间漂移达2.33，而当储备池更新步长随温度按匹配关系调整后，漂移压缩至0.17。这给出一个简单的器件操作配方：无需改变器件，只需令系统时钟跟随$$\tau(T)$$，即可在宽温区保持接近最优的性能。该结论也呼应第二章的器件设计，若目标工作温区已知，可据匹配关系反推应制造的势垒高度。

上述温轴采用固定势垒高度的简化，即$$\Delta(T)=\Delta_{300}\cdot 300/T$$只随分母kT缩放；把第四章建立的磁性标度链引入后，势垒本身也随温度下降 (界面项与形状项的近补偿之差被温度放大)，修正后的$$\tau(T)$$在高温端明显更短，400 K处约为简化口径的一半[^tau_axis_note]。热时钟配方的形式不受影响——系统时钟跟随$$\tau(T)$$即可，无论其取何口径——但按温反推势垒或设定重标定周期时应使用修正温轴。修正轴与第四章概率二值网络的部署温窗共享同一标度链，两种工作模式的温度行为由此统一在同一温轴上 (第四章图4.24)。

[^tau_axis_note]: 此处记录一次口径订正：本节初版温度研究即采用仅kT缩放的简化轴，第四章温度链完成后经交叉核对，确认高温端弛豫时间被简化口径高估约2倍；图5.4的定性结论 (强敏感性与时钟补偿有效性) 不受影响，定量温轴以修正口径为准。
...
同一阵列的PBNN模式以1比特感测加popcount计数器读列，一个自然的设想是储备池模式复用这套计数外围、省去逐次逼近转换器，物理系综台架上的同列对比 (图5.11) 排除了这条看似显然的路线。积分窗与弛豫时间之比决定窗内时间平均的价值：步长25 ns、弛豫时间数十纳秒时每窗仅约1.2至1.6个独立样本，分辨率只能来自系综规模，每增加一个有效比特需四倍器件、也需四倍感测能量 ($$b_\mathrm{eff}=\tfrac{1}{2}\log_2 E$$)，而逐次逼近的代价只随位数线性增长。在标准系综规模96处，单外围计数系统 (递归与读出同用一次popcount选通) 的记忆容量为1.59、低于散粒噪声匹配的逐次逼近读出的2.33，读出能量却高约24倍 (按平均递增计费约20倍)；单次选通还放弃了模拟积分器无代价收集的窗内积分增益。计数读出因此仅在不允许任何模数转换器的面积最小单外围约束下存活，凡列共享逐次逼近可用之处均被排除。该对比同时给出一个校准侧发现：同一工作点的均值场记忆容量约4.9是无穷系综上限，物理系综的记忆容量由系综噪声经读出通道限制 (递归通道的噪声贡献经消融确认在估计噪声内)，此前基于无噪声均值场信号的等能量前沿因而应读作上限而非可达值。
