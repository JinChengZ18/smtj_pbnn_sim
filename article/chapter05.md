...
上述能量对比把读出连同模数转换计入，其底层是读出与阵列的协同设计[^ro_costmodel]。把模数转换能量以提取的sky130 StrongARM比较器作逐次逼近转换器的比较器项、电容数模阵列的开关能量以sky130单位电容标定后，读出主导储备池能量：比较器即使3位分辨也已占总能量近九成 (图5.9)；但比较器项随位数线性增长，分辨率的能量惩罚温和。对集成式储备池而言低至中等分辨读出即足这一结论本身在近期工作中已有报道[^ens_rc]，本章区别于该工作之处在于以记忆容量每焦耳为目标、把节点数纳入联合权衡，并采用列共享、时分复用的逐次逼近转换器，把一套比较器与电容阵列摊销到超顺磁电报噪声节点之上 (图5.8)。正是这一摊销使前述能量优势在含提取读出时仍达约30倍 (逐节点8位) 至约35倍 (列共享)，而非以降低分辨率换取。
