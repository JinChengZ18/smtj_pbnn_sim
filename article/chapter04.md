...
上述外围模块最终落到存储单元的版图与集成结构上 (图4.22)。2T单元由写、读两只访问管与其金属引出构成：写访问管须通过约1.16 mA写电流、取约2.2 µm管宽，读访问管取最小宽度；单元版图以工艺套件的参数化单元脚本化生成，CMOS部分经官方设计规则集检查零违例[^drc_control]，寄生提取还原出两只晶体管的准确尺寸与约3 fF的单元内互连电容。磁隧道结柱与SOT沟道无开源工艺可承载，依混合CMOS/磁性工艺套件的既定做法[^hybrid_mpdk]以注记层黑盒表示：黑盒插于met2与met3之间，底电极焊盘与源线落在met2 (与写线走met2及以上的设计规则相衔接)，顶电极经met3读线引出；其平面几何以公开报道的器件外形为锚，即柱临界尺寸约80 nm、沟道宽200 nm、底电极间距200 nm[^hikstor_edl]，垂直堆叠细节不能从公开信息复现、保持晶圆标定输入的身份。实绘单元含字线、位线引出与独立体接触条，设计层包络约22.7 µm²，从上方界定4.6 µm²的阵列摊销估算，二者之差即单元级引出与共享体接触、邻接布线之间的口径差。SOT层材料的自旋霍尔角公开报道约为−0.3 (钨，4 nm)[^hikstor_edl][^beta_w]，第二章标定的器件级有效自旋霍尔效率显著低于该材料值，差距源于自旋透明度与电流分流等界面及几何因子，引用时两种口径不可混用。
...
器件结构参数组本身也可作自洽核验 (图4.23)。以第二章的界面各向异性、饱和磁化、自由层厚度与电学直径正演保持势垒 (含精确扁椭球退磁因子)，得约48.5 kT，与同平台记忆级器件公开报道的保持带59–64 kT同量级；有效各向异性是界面项与形状项两大数之差、补偿约81%，界面各向异性每变化1%即令保持势垒移动约5.3%，这一近补偿放大既把与公开带的差距归结为百分之几的叠层参数精度，也定量解释了标定所得约7.7%的器件间热稳定因子离散仅需约1.4%的微观涨落。此处正演的是保持势垒，与写概率标定的开关律指数$$\Delta=4.91$$分属不同物理量；同一叠层要把保持势垒压到超顺磁工作点，需把电学直径缩到约17 nm或把界面各向异性修剪约10%，故超顺磁变体更可能来自近补偿叠层的微调而非几何缩小。单元间磁串扰由自由层磁矩的偶极场界定：磁矩仅约$$2.3\times10^{-18}\,\mathrm{A\cdot m^2}$$，最坏情形 (近邻全同态、含点阵求和裕量) 下概率工作点的偏移在实绘单元间距处为$$10^{-6}$$量级，黑盒抽象在磁学上自洽；偏移达1%的临界间距约211 nm，仅无访问管的密集后段阵列才进入需预算区，较常规确定性存储约30 nm即可的同类间距规则[^mram_pitch]严约一个数量级——低势垒器件对杂散场的指数敏感把磁串扰间距要求推入亚微米，但仍远小于访问管决定的微米级单元间距。器件内固定层的静态杂散场由合成反铁磁叠层在设计上补偿，不属此处的单元间耦合。
