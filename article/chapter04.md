...
**图4.15** (a) sky130 StrongARM输入折合失调 ($$0.39\,V_T$$) 与判决窗$$V_T$$的相对关系，按$$V_T$$而非磁阻余量预算失调。(b) 扇入1024、$$V_\mathrm{in}=0.5\,\mathrm V$$下四种失调消除方案的精度跌幅对剩余失调，除最低压宽扇入区外跌幅均在统计涨落内，平凡比较器位于帕累托前沿。(c) 三种读出比较器 (本文StrongARM、double-tail、DSA) 在同一sky130失调蒙特卡罗夹具下的输入折合失调 ($$\sigma_\mathrm{off}/V_T$$，误差棒为$$\pm1$$标准误)：三者在130 nm下同落于约$$0.39\,V_T$$且误差棒相互重叠，文献在更先进节点报告的拓扑失调优势在此并不复现，故器件最少的单级StrongARM为帕累托最优。与未在本地复现的更广文献的定性能力对照见附录D。
