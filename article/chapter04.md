...
**图4.18** 沿高列的写通路设计与设计空间对比。(a) 无补偿时单元写电压随行号线性下降，预畸变使各行均见目标电压。(b) 无补偿时写概率由近端0.90塌陷至远端0.016，预畸变将其在每行拉平到目标。(c) 本写通路与近期文献多方案的对比：横轴为$$N=256$$高列的剩余远端写电压误差、纵轴为电路层实现完整度 (数模转换器拓扑＋稳压轨＋能量账)，本文 (星标) 兼得低剩余误差与高实现完整度，文献方案或停在模型/编码层、或为单器件 (数据来自多智能体arXiv/开放获取调研)。
