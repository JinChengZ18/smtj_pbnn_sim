...
![图4.1 sMTJ-PBNN单层闭环前向的硬件实现示意](pic/Chapter04_local_01.png)
...
![图4.2 分层硬件仿真器的模块组织](pic/Chapter04_local_02.png)
...
![图4.3 sMTJ写入概率的Sigmoid拟合与NB桥式联合曲面](pic/Chapter04_local_03.png)
...
![图4.4 MNIST上PBNN-MLP的端到端验证](pic/Chapter04_local_04.png)
...
**表4.2** MNIST上同拓扑PBNN-MLP与QAT量化FP-MLP的最佳测试精度。
...
![图4.5 PBNN-MLP在UCI六类表格任务上的训练曲线](pic/Chapter04_local_05.png)
...
![图4.6 优化器与学习率调度的对比](pic/Chapter04_local_06.png)
...
![图4.7 PBNN-MLP的损失景观与checkpoint轨迹](pic/Chapter04_local_07.png)
...
![图4.8 八类输入扰动在单个MNIST样本上的可视化](pic/Chapter04_local_08.png)
...
![图4.9 PBNN、BNN、FP-NN在八类扰动下的精度衰减](pic/Chapter04_local_09.png)
...
![图4.10 概率二值编码与数字MRAM位编码的比特价值对比](pic/Chapter04_local_10.png)
...
![图4.11 硬件比特翻转鲁棒性扫描](pic/Chapter04_local_11.png)
...
![图4.12 非理想性对sMTJ Sigmoid响应曲线的影响](pic/Chapter04_local_12.png)
...
![图4.13 非理想性消融下的测试精度](pic/Chapter04_local_13.png)
...
![图4.14 九种存储器架构在20轮MNIST PBNN-MLP训练下的总能耗对比](pic/Chapter04_local_14.png)
...
本章构建了面向sMTJ-PBNN的端到端硬件仿真器，其关键设计是让软件基线、硬件感知和全栈评估三档模式共享同一份潜参数检查点。三档模式既共用$\boldsymbol{\Theta}$张量，也借助硬二值STE技巧复用BN滑动统计，因此训练所得检查点无需重新训练即可切换到真实硬件语义下评估。这一设计降低了训练与推理语义不一致带来的偏差，使仿真器能够同时服务于训练循环、PPA估算和鲁棒性扫描。与此同时，CLT高斯化前向将训练阶段的$T$次显式Bernoulli抽样压缩为一次解析近似，使每步训练复杂度回到与确定性矩阵乘法相近的量级，为PBNN的可训练性提供了工程基础。

鲁棒性分析的核心并不只是某一扰动强度下的精度数值，而是PBNN相对FP位置编码具有结构性优势。FP编码中高位承担更大动态范围，MSB翻转会造成远大于低位翻转的误差；PBNN则将权重展开为$T$个独立Bernoulli样本，每个样本对输出的贡献幅度均受$2/T$约束，因此单点翻转的最坏影响由编码方式直接限制。该机制解释了PBNN在比特翻转扫描中的稳定性，也表明其可靠性收益并非依赖额外纠错电路。非理想性消融也给出了清晰的工程结论：影响精度的主要瓶颈是$V_\mathrm{th}$的绝对位置稳定性，而斜率漂移、循环噪声与back-hopping可分别通过BN、$T$步平均和工作点回退缓解。硬件优化因而可优先聚焦于DAC校准与温漂补偿。
...
总体而言，本章表明sMTJ-PBNN在能效上可与优选的确定性CIM保持同量级，在精度上付出可量化的小幅代价，并在鲁棒性上获得来自编码机制的优势。这三方面结果共同构成后续系统设计与取舍分析的依据。
