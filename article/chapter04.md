...
**图4.23** 器件结构参数组的自洽核验与阵列磁串扰界。(a) 由结构参数组正演的保持势垒随电学直径的变化：标定直径处约48.5 kT，与公开报道的记忆级保持带同量级，压到超顺磁开关律指数所在水平需缩至约17 nm；(b) 最坏情形近邻概率偏移随阵列间距的变化：偏移1%的临界间距约211 nm，实绘单元间距下偏移约$$10^{-6}$$，磁串扰仅在无访问管的密集后段阵列域构成约束。
