...
**图4.18** 沿高列的写通路设计与文献能力对照。(a) 无补偿时单元写电压随行号线性下降、远端约148 mV跌破标定$$V_\mathrm{th}$$ (红色阴影区写入失败)，IR感知预畸变使各行均见目标电压。(b) 在三个目标工作点 ($$P_\mathrm{target}=0.5/0.9/0.99$$) 下，无补偿写概率均沿列塌陷，预畸变把每行$$P_\mathrm{sw}$$拉平到各自目标 (虚线)。(c) 写通路与近期文献多方案的能力矩阵：逐项标注各设计是否提供逐行IR预畸变、具体DAC拓扑与稳压轨、写能量账、二值SOT $$P_\mathrm{sw}$$目标、逐列$$V_\mathrm{th}$$微调及sMTJ器件级落地，本文为唯一在电路层将各半合一者，文献方案或停在模型/编码层、或为单器件 (定性对照；各方案引文见submodule_survey数据集)。
