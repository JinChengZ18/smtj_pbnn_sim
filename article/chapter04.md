...
由上述两点，PBNN在MRAM-CIM架构内的前向传播被重构为物理与数学严格对应的闭环迭代。上一层生成的随机二值向量$$\boldsymbol{x}^{(r)}$$驱动MRAM阵列与固化权重$$\boldsymbol{W}$$执行XNOR-CIM运算，列电流向量正比于$$\boldsymbol{a}^{(r)}=\boldsymbol{W}\boldsymbol{x}^{(r)}$$；列电流先经斜率匹配读出（跨阻$$R_\mathrm{TI}$$并接StrongARM比较器作概率位判决，或列共享逐次逼近转换器作储备池多位读出）转为数字量，再由概率映射与写通路（电压型电阻串写数模转换器、IR感知逐行预畸变与CMOS写驱动）把$$\boldsymbol{a}^{(r)}$$映射为下一层目标翻转概率$$\boldsymbol{p}^{(r+1)}=g(\boldsymbol{a}^{(r)})$$并生成对应的物理写入激励；写入激励驱动后级采样MTJ阵列发生热随机翻转，输出新一轮独立状态$$\boldsymbol{x}^{(r+1)}\sim 2\,\mathrm{Bernoulli}(\boldsymbol{p}^{(r+1)})-\boldsymbol{1}$$。第一次迭代的输入$$\boldsymbol{x}^{(0)}$$来自确定性原始数据 (如图像像素经量化后的二值编码)，随机采样从第二层起逐层引入。该闭环前向的硬件实现如图4.1所示。

![图4.1 sMTJ-PBNN单层闭环前向的硬件实现示意](figs/Chapter04_local_01.png)

**图4.1** sMTJ-PBNN单层闭环前向的硬件实现，按实际电路设计绘制。左侧2T SOT-MTJ存内计算阵列承担确定性二值权重$$\boldsymbol{W}\in\{-1,+1\}$$，输入二值向量$$\boldsymbol{x}^{(r)}$$经位线电压编码后，列电流由基尔霍夫电流定律自动完成XNOR-popcount，得到$$\boldsymbol{a}^{(r)}=\boldsymbol{W}\boldsymbol{x}^{(r)}$$；斜率匹配读出（跨阻$$R_\mathrm{TI}$$并接StrongARM比较器作概率位二值判决，或列共享逐次逼近转换器作储备池多位读出）将列电流转为数字量；概率映射与写通路（电压型电阻串写数模转换器、IR感知逐行预畸变、CMOS写驱动）据此生成下一层目标翻转概率$$p=g(a)$$对应的物理写脉冲；右侧随机SOT-MTJ采样阵列在亚临界写脉冲下发生热涨落翻转，输出新一轮Bernoulli样本$$\boldsymbol{x}^{(r+1)}$$闭环反馈；对$$T$$次独立样本作空间或时域平均即近似恢复推理期望$$\mathbb{E}[s]$$。该结构把概率神经元的随机熵源与权重计算单元在同一阵列内原位融合，无需独立TRNG模块；各功能块均已在开源sky130工艺上完成器件级电路设计与晶体管级验证。
