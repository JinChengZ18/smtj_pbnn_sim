...
$$T=4$$时测试精度即达到97.51%，与$$T=64$$的渐近上限97.68%相差仅0.17个百分点[^seed_band]；$$T=8$$进一步收敛至97.62%，此后的额外采样收益小于0.1个百分点而能耗按线性递增。因此后文的鲁棒性与能耗对比中默认采用$$T=4$$作为部署目标[^layer_T]，这一选择把PBNN的推理能耗压缩至$$T=64$$版本的十六分之一，而精度损失在测量噪声以内。$$T=1$$已能给出96.91%的精度，原因是后训练时把潜参数$$\theta$$作了乘100的标度处理，使得$$\sigma(\theta)$$几乎都饱和在0或1，Bernoulli样本接近确定性，$$T$$主要补偿那些$$\theta$$仍处于0附近的少数权重。

[^seed_band]: 附录C的8种子重训练把这些单实例数字放进种子带：最佳全栈精度97.01±0.17%、$$T=64$$与$$T=4$$之差0.21±0.11个百分点，正文数字均落带内；小于该带宽的相邻差值 (如表4.2中PBNN与确定性BNN的0.07个百分点) 应读作统计不可分辨。同种子配对重训 (8种子分别训练PBNN、BNN与FP32各20轮，图4.8的基线配方、不注入器件失配，同种子相减消去共同训练方差) 进一步给出：$$T=64$$与$$T=4$$之差+0.15±0.08个百分点，与正文的0.17一致；PBNN与BNN之差+0.39±0.25个百分点，方向与表4.2实例相反——半个百分点以内的相邻排序随训练口径翻转，不承载结论；FP32与PBNN之差+1.03±0.17个百分点，量级稳定，把二值权重容量计为主要差距来源的归因不变。

[^layer_T]: 采样数按层再分配不改变该部署选择：三层的每MAC能量成本相差两个数量级 (分类层仅占总MAC数的0.6%)，但在总能量固定下把大层的采样让给近乎零成本的分类层 (乃至充至184次) 的七种调度，其精度全部落在均匀调度的±0.1个百分点噪声带内；各层对采样数的边际收益在$$T\ge 2$$后即趋平坦。故$$T$$作为全局参数报告，不引入逐层调度。CLT路径与显式$$T$$步采样的一致性在合成数据上由单元测试直接验证：随机生成$$M=64,N=256,B=8$$的概率张量，CLT解析均值与500次显式Bernoulli样本的均值差异z-score在所有元素上均小于5；CLT输出的标准差按$$\sqrt{N}$$增长，与理论一致。在MNIST PBNN-MLP上，软件档 (理想Sigmoid) 训练得到的检查点经$$\theta\times 100$$标度后再用全栈$$T=4$$评估，精度从硬件感知训练时的96.98%回升至97.51%，差距与Bernoulli样本数从无穷大降到$$T=4$$的截断误差量级一致。

固定采样深度对每个输入付出同样的样本数，而判决所需的证据量随输入而异。把逐时隙到来的样本视作一列投票、在领先类与次席类的票差达到给定裕量时停止，即可在同一时序上实现按输入自适应的采样：平均2.1次采样达到固定$$T=4$$的精度，平均6.7次即取到固定调度需$$T=32$$以上才收敛的渐近精度。停止统计量就是计数式读出已经产生的票数，不需要额外电路，代价是采样数按输入涨落，票差裕量取6时95%的输入在8次采样内完成，长尾输入继续采样至上限。该结果与逐层预算再分配的零收益并置：采样预算在层间重排无效，在样本之间重排有效[^adaptive_stop]。

[^adaptive_stop]: 停止规则与共形预测的联动经检验不成立，一并记录于此。为保住分裂共形的覆盖保证，停止规则先在校准集上运行、共形分位数取停止时刻的非一致性分数，因而整个自适应预测器作为一个固定过程被校准；实测结果是该任务上的共形集合退化为单元素，覆盖率等于top-1精度，集合值预测不承载额外信息，故正文只引用采样数与精度。评估以测试集前半为校准、后半为评测，正文引用的精度为全测试集口径。
...
对四种网络架构 (PBNN $$T=4$$、PBNN $$T=64$$、确定性BNN、全精度FP-NN，共享拓扑$$784\to 1024\to 1024\to 10$$) 施加八种扰动，包括加性高斯噪声、椒盐噪声、speckle乘性噪声、高斯模糊、cutout遮挡、亮度位移、权重高斯扰动与十步投影梯度下降 (Projected Gradient Descent，PGD) 对抗攻击。八类扰动在同一MNIST样本上的可视化如图4.7所示，三种架构在八类扰动连续扫描下的精度衰减汇总于图4.8与表4.4。
...
| PGD-10 | $$\epsilon=0.1$$ | 53.22 | 50.03 | 36.85 |

从定性角度，FP-NN在保留输入分布的扰动 (高斯、椒盐、speckle、cutout) 上有可测的优势，因为连续权重对小幅度线性扰动的吸收最为有效；PBNN在扭曲输入分布的扰动 (模糊、亮度位移) 上反超，因为二值权重对像素绝对值的依赖更弱；在权重空间扰动和PGD对抗攻击上，PBNN的优势最为显著。在权重扰动$$\sigma_w=0.5$$这一更严苛的工况下，PBNN $$T=4$$仍保持93%以上，而BNN降至9.35%、FP-NN降至14%，体现了stochastic averaging对独立权重扰动的天然抑制：$$T$$次采样把每个权重的方差除以$$T$$。在PGD对抗攻击下，PBNN比FP-NN高出约15个百分点[^eot_audit]；适应性攻击审计与BNN对照共同表明，该优势主要来自sign路径的二值化，前向采样随机性的贡献有限——对随机前向做梯度平均的期望变换 (Expectation over Transformation，EOT) 攻击并不比确定性CLT替代梯度更强，因为后者正是随机前向的解析期望。

[^eot_audit]: 按随机化防御的评测规范[^eot_athalye][^eot_tramer]对本行做了适应性攻击审计。表中PBNN值为修正口径：攻击梯度取CLT替代路径 (随机前向的解析期望，等价于采样数趋于无穷的EOT梯度)，终评统一为全栈$$T=4$$；修正前的终评误用CLT路径，规范数据为52.12%，同一检查点重测得旧口径52.5%、修正口径53.2% (攻击实例间浮动约0.4个百分点)。审计结论：对全栈随机前向直接做EOT-PGD (K=10–20、10–40步) 得52.2%–52.9%，不强于CLT替代攻击；最强预算 (40步、K=20、3次随机重启取最坏) 下为45.4%，仍高于FP-NN同预算白盒的43.4%；自FP-NN与BNN的迁移攻击 (53.7%、53.9%) 不强于白盒。以相同配方重训的基线实例给出BNN 54.4%、FP-NN 43.4%，即对抗精度对训练实例敏感，PBNN对FP-NN的差距在10至16个百分点间随实例浮动而方向稳定；BNN与PBNN在攻击下同档，亦印证优势主要源于二值化。
...
把两种编码暴露在均匀单比特翻转概率$$p$$下，得到表4.5与图4.10的精度对比。差距在$$p\ge 0.05$$时显现：在5%翻转率下PBNN $$T=64$$仍保持97.45%，而FP-NN降至92.44%；在10%翻转率下FP-NN降至52.32%，PBNN $$T=64$$保持96.73%，差距达44个百分点；该差距对配备SECDED纠错的数字基线同样成立——单纠双检在低翻转区 ($$p\le 0.005$$) 把数字精度完全修复到干净基线，但修复能力在$$p\approx 0.01$$处已折半，$$p=0.1$$时每72位码字期望7.2个错误、纠错饱和 (52.37%)，故优势源于编码结构差异而非缺少纠错。把单MSB翻转作为极端情形：每个权重上翻转最高位时，FP-NN的精度从98.41% (仅翻最低位) 降至3.41% (翻最高位)，95个百分点的差距源于位置编码下不同位的非等权地位；PBNN的同等极端情形是把所有$$T$$个样本一起翻转，但单个样本翻转的影响只有$$2/T$$。这一观察解释了为何PBNN $$T=64$$在$$p=0.05$$下的有效错误幅度落在以0.1为中心的窄分布内，而FP-NN的有效错误幅度呈长拖尾，最大值可达1.5倍满量程。

**表4.5** 均匀单比特翻转率下不同权重编码的MNIST测试精度。

| $$p_\mathrm{flip}$$ | PBNN $$T=8$$ | PBNN $$T=64$$ | BNN (1比特) | FP-NN (8比特) | FP-NN+SECDED[^ecc_baseline] |
|---|---|---|---|---|---|
| 0.000 | 97.55 | 97.59 | 96.59 | 98.42 | 98.42 |
| 0.020 | 97.31 | 97.66 | 96.28 | 97.65 | 97.86 |
| 0.050 | 97.30 | 97.45 | 95.06 | 92.44 | 92.67 |
| 0.100 | 96.26 | 96.73 | 91.22 | 52.32 | 52.37 |

[^ecc_baseline]: SECDED取Hamming(72,64)：每72位码字纠1检2，存储位开销12.5% (解码逻辑的能量与延迟未计入)。其作用等价于把每位翻转率$$p$$映射为残余率$$p_\mathrm{eff}=\sum_{k\ge 2}k\binom{72}{k}p^k(1-p)^{72-k}/72$$ (不可纠码字保留全部错误位，忽略奇数错的误纠增位、对数字基线略偏有利)；受保护精度即裸INT8实测曲线在$$p_\mathrm{eff}$$处的取值，与表中FP-NN列为同一模型实例。
...
位翻转下的精度可由经验观察提升为闭式可认证的结论，根据是翻转信道把受损网络保持在同一模型类内。以$$q$$记单元取$$+1$$的潜概率，速率为$$p$$的独立同分布符号翻转把该单元精确映射为另一个伯努利变量，$$q'=(1-2p)q+p$$；各单元相互独立，该映射在全网逐层同时成立，受损网络在分布意义上等同于把所有潜概率向$$1/2$$收缩后的干净网络。位置编码不存在这一约化，INT8下翻转后的权重仍是合法权重，但偏移幅度随被翻位次按$$2^k$$放大，整体扰动写不成对干净参数的单参数变换，只能逐实例经验评测。收缩等价把认证问题化为对收缩后参数的尾概率估计：固定末层输入的二值特征后，真类与任一竞争类的判决边际是若干独立三值变量之和，其均值、方差与三阶绝对中心矩均可解析写出，误判尾概率取Bernstein界与高斯尾加Berry–Esseen余项 (常数取0.56[^shevtsova2010]) 的较小者，对竞争类取并集并对特征取全期望，即得端到端的精度下界[^cert_scope]。

该下界紧贴经验曲线，表明这一约化没有以界的松弛为代价：$$T=64$$在全翻转率网格上认证值与实测值相差不超过1.3个百分点，采样数减少则判决边际的相对涨落变大、尾界转保守，$$T=8$$在$$p=0.2$$处扩大到3.5个百分点。界的工程价值在于可反解出翻转率预算：以干净工作点为基准允许1个百分点的认证精度退化时，$$T=8$$容许7.9%的单元翻转率，$$T=64$$容许9.0%，该数值可作为保持特性与读扰动的设计规格直接引用，无需对特定翻转实例重复评测[^cert_ci]。

[^cert_scope]: 认证的信道口径为逐次抽样刷新的翻转，即每权重的$$T$$次采样各自独立翻转，与本节经验扫描的实现一致；若翻转为淬火式缺陷 (单元极性被永久反转、每次前向复现同一反转)，其单元均值与收缩结果相同，但误差在样本之间相关，不在该分布等价的覆盖范围内。批归一化统计量在干净工作点标定后冻结，不随$$p$$重新标定。闭式推导假定单元为静态伯努利，因而该扫描关闭C2C循环噪声与back-hopping平台钳位两个通道 (钳位与收缩变换不对易)。
[^cert_ci]: 认证值是对特征抽取取全期望后的蒙特卡洛估计，报告值附8次抽取的单侧95%下置信沿，其地位是带置信区间的估计而非确定性证书；该实验在4000样本的测试子集上评估，绝对精度与表4.5的全测试集口径不可直接比较，故正文只引用同一次运行内认证值与实测值之差，0.5个百分点一档的反解落在该子集约0.3个百分点的二项标准误内、不足以分辨。推导过程有一次方向性修正：初版把翻转扰动当作干净边际上的零均值噪声并套用Hoeffding不等式，所得界平凡成立而无信息，原因是扰动携带系统性的收缩项；保留收缩后的精确均值、只对真实涨落设界之后，界才转为有效。

同一批采样还承载一条不额外耗电的用途：把$$T$$次采样当作集成成员，可由同一次前向得到预测分布的不确定度分解。校准误差随采样数单调改善，$$M\ge 8$$即优于同任务全精度网络的softmax置信度，且远优于确定性BNN；按预测熵排序保留90%覆盖时，测试精度升至99%以上。分布外检测则不成立：互信息给出的认知不确定度随采样数下降，改用更难的CIFAR-10任务与器件实例集成两条路线复验，均未使其恢复[^uq_scope]。该用法的定位因而是校准与选择性预测。

[^uq_scope]: 两条复验路线的结果。器件实例集成 (每个成员重抽标定$$\mathrm{CV}(\Delta)=7.7\%$$的器件场) 与标称场全程重合，分布外AUROC同样随采样数下降；CIFAR-10上分布外AUROC随采样数上升但绝对值不超过0.74，高斯噪声位移通道无信号，且该检查点的潜概率饱和程度与MNIST检查点相当 (中段占比18.1%对14.9%)，以饱和解释分布外信号缺失因而未获支持。深层二值CNN上另有一项代价：把同一采样预算用作外层集成而非层内平均，精度低4至9个百分点，而MNIST-MLP上二者等同，说明不确定度轴在浅层网络上无代价、在深层网络上并非如此。
...
图4.12把硬件设计的优先级清晰地指向DAC校准精度：$$V_T$$的slope抖动会被BN自动吸收，C2C噪声会被$$T$$步平均消除，真正决定网络精度的是$$V_\mathrm{th}$$的绝对位置稳定性[^vth_drift]。在此之上，给出sMTJ对单次MAC的能量分解。外围三项 (读出、DAC、计数器) 均由4.6节的sky130提取与估算给出：DAC编程约34 fJ、sMTJ SOT写$$0.78\,\mathrm{pJ}$$ (有物理依据)、sMTJ读约48 fJ、计数器累加约19 fJ，累计约884 fJ每MAC，sMTJ写占约89%[^te_read]。写仍主导能量，故任何缩短脉冲宽度、降低写电压或增大$$R_\mathrm{SOT}$$的器件改进都会按$$V^2 t/R$$线性回报到全网能耗；但外围三项落实后合占约11%、已非可忽略，读出电路的能量—精度协同因而进入优化视野 (详见4.6节)。
...
[^vth_drift]: 该静态结论延伸到时间轴的漂移压力测试给出定量边界：在阈值场上注入逐块累积的随机游走，0.5个百分点的精度预算对应每列共模约1.0倍判决窗、每单元独立约2.4倍判决窗的漂移容限，列级系统性漂移是约束模式；再校准节拍遵循随机游走标度，即容限与单块步幅之比的平方。策略对比显示批归一化重校准对列漂移几乎无效 (漂移是判决阈值的位置偏移而非群体统计漂移)，而写数模转换器的每列微调码刷新以每次约70 pJ的近零代价把精度保持在基线带内，使该微调同时承担失配修正与老化补偿两重职责。

每样本仅计一次写脉冲，前提是采样时序不含复位相；三种候选时序中，读后择极性是唯一同时保持样本独立与单脉冲写代价的一种。sMTJ零偏下的弛豫时间约68 ns，远长于4 ns的采样时隙，单元在相邻时隙间几乎不遗忘上一次状态，去掉复位相就把独立采样变成链上采样。既不复位也不读态的单极性脉冲串把状态棘轮到脉冲偏好侧，平稳分布对任意$$\theta$$不低于1/2，权重的负半平面不可达；按权重符号静态选择极性可覆盖两个半平面，但须对编程电压预畸变才能复原目标边缘分布，而预畸变后中段单元的时隙间自相关达0.95、$$T=4$$平均的方差膨胀约3.8倍，精度较复位基线低0.7个百分点。读后择极性以写前的一次感测确定脉冲极性，条件切换概率精确实现目标边缘分布、样本严格独立，所需的双向切换标定即第二章的两方向实测曲线；其精度与复位基线之差落在采样噪声内，而写路能量由每样本两次脉冲降为一次脉冲加一次感测，减少46.9%。该结论在标定失配下保持：注入变异系数7.7%的势垒离散并抽取8个器件实例，读后择极性与承受同一写误差的复位基线之差为$$+0.16\pm0.34$$个百分点，统计上不可分辨[^sample_timing]。先复位再编程的时序会使单次MAC能量上升约90%，本节的跨架构能耗对比因而以读后择极性为计费前提。

[^sample_timing]: 该计费口径经历过一次自查订正。能量模型自始按每样本一次写脉冲计费，而其默认设想的时序是先复位到已知态再编程，后者需要两次脉冲，二者并不自洽。把三种时序与计费逐项对照后确认，一写一读的现行口径对应的是读后择极性：相对纯写计费只增加写前的一次感测 (约48 fJ)，而先复位再编程增加的是第二次写脉冲 (约0.78 pJ)。器件失配下的对照取一阶口径，势垒离散经Néel-Brown关系折算为逐单元的阈值与判决窗偏移，并以共模形式同时施加于两种脉冲极性，切换方向间的不对称与非共模项不在该口径内。

漂移压力测试给出的是抽象的共模预算，把第二章的磁性温度标度律沿校准链传播则给出其物理刻度：环境温度是最主要的共模轴。有效各向异性是界面项与形状项约81%补偿之差，二者随温度以不同指数下降 (Bloch $$T^{3/2}$$对修正Callen–Callen指数2.18)，升温使补偿加深、$$K_\mathrm{eff}$$的温度斜率被近补偿放大，由此传播到写阈值，得$$\mathrm{d}V_\mathrm{th}/\mathrm{d}T\approx-2.1\,\mathrm{mV/K}$$，而判决窗仅按$$V_T(T)=V_{T,300}\cdot T/300$$缓慢展宽 (补偿比在临界电压与开关指数之间严格相消)。将对应的共模变换$$p\to\sigma(a\theta+b)$$ ($$a=300/T$$，$$b$$由阈值漂移折算) 施加于已训练网络的全部单元，得到部署温窗：不作任何重标定时，精度损失1个百分点的窗口仅约[293, 309] K；一次共模刷新 (清零$$b$$) 后，网络在250至400 K全程平坦——温窗受共模失调而非斜率失配限制。

这一结论把温度补偿职责清晰分层：温漂应由全局写电压轨的单点校正吸收 (400 K处需约−214 mV，超出每列微调码量程)，每列微调码专门修正器件间失配；该路线与在权重空间吸收器件参数偏移的阵列级补偿[^tcomp_weight]互补，后者需重新求解权重，前者复用既有数模转换电路。

器件级$$P_\mathrm{sw}(T)$$与$$\tau(T)$$行为已有实验确立[^smtj_temp_apex]，网络级性能对热稳定体制的敏感性也有先例[^ssn_regimes]；此处的贡献是把单点标定锚与平台标度律串成一条从叠层参数到部署温窗的可复算链，并给出链的不确定性带：温窗数值依赖超顺磁变体的补偿实现 (原生叠层0.81、修剪路线0.98、缩径路线0.73) 与Callen–Callen指数的文献带$$n\in[1.8,2.8]$$，1个百分点温窗最宽至[279, 327] K、最窄至亚开尔文量级；悲观端为热激活外推，乐观端有纳秒弹道区阈值弱温度依赖的实验支持[^rehm_ballistic] (0.75 ns工作脉宽恰处过渡区，两端界定真实器件所在区间)。全LLG动力学在270至330 K对临界电流温度比值的独立复算与解析链在蒙特卡洛不确定度 (约1%) 内一致，构成该链的双模型锚。整条标度链、部署温窗与储备池热时钟的修正温轴 (见第五章) 如图4.24所示。
...
外围CMOS选用sky130 (130 nm/1.8 V)，因为它是目前唯一同时提供器件模型与完整开源EDA工具链 (ngspice、Magic、KLayout、Netgen) 的工艺套件；各项数值的提取方式分别为：灵敏放大器的输入折合失调由ngspice对器件阈值失配作蒙特卡洛得到，其判决能量由Magic提取的器件电容结合ngspice瞬态积分得到，写线寄生电阻与IR压降由Magic电阻提取[^extresist]标定各层方块电阻后按列几何标度得到，器件本身由OpenVAF编译的Verilog-A紧凑模型在ngspice中调用。磁隧道结是后段 (BEOL) 集成的器件，其临界尺寸 (约80 nm)、隧穿磁阻、写电流与写能量由第二章晶圆标定给出，与CMOS逻辑节点无关：这与近期实测的无沟道SOT-MRAM工艺[^hikstor][^hikstor_data] (临界尺寸约100 nm) 在量级上一致，本仿真的器件级写能正落在其2至5 ns工作点之间；区别仅在于势垒调校：该存储器取高保持 (Δ=50–55)，本文的概率位与储备池应用则采用低势垒 (Δ≈4.9) 的超顺磁变体。近期基于电压控磁隧道结的概率计算集成电路即在130 nm CMOS上实测[^pbit_asic]，可见该节点对此类器件是已被验证的工程选择；而商用磁存储器的外围CMOS通常在22至28 nm节点，故sky130的130 nm外围在能量、面积与速度上是保守上界。本节因此在绝对值之外一律以比值[^ratio_metrics]报告结论。

[^extresist]: Magic的电阻提取(extresist)经自校验给出多晶硅48.0 Ω/□，与工艺值48.2 Ω/□相差0.5%。

[^hikstor_data]: 该工艺为浙江驰拓科技在300 mm晶圆上实测的无沟道SOT-MRAM，关键指标为约100 nm临界尺寸、115% TMR、耐久>$$10^{12}$$。
[^ratio_metrics]: 此处的比值指失调比判决窗、能量占比与写线IR占器件比三项。

器件侧并行保留两套模型，一套是对实测数据回归的代数紧凑模型，承担全部电路与系统迭代，另一套是宏自旋Landau–Lifshitz–Gilbert随机动力学求解器，物理上更完整、计算量更大，专用于交叉验证；在0.75 ns自旋轨道矩写脉冲下两模型的判决阈值相差约9 mV (约0.4倍判决窗$$V_T=23.4\,\mathrm{mV}$$，图4.21)，仍足以支持以代数模型驱动整条设计流程。宏自旋LLG的概率曲线斜率较实测Sigmoid更平缓[^llg_errata]——单宏自旋模型与Néel-Brown一致地低估了循环间收窄 (C2C) 后的实测斜率 (对应收窄因子$$\eta_c$$所刻画的缺口)；两模型在判决阈值上吻合，过渡斜率的差异属预期的亚畴协同效应。由这些器件级数值驱动的外围在片上层面经位线、源线、字线与读线，与写通路、读通路、行列译码及模式与时序控制器相连 (整体架构层次见第五章图5.10)。

[^llg_errata]: 这一斜率差异在开发中期才显现：宏自旋求解器的早期实现存在场样自旋轨道矩积分的一致性错误，人为锐化的概率曲线曾使两模型的斜率看似一致；勘误后判决阈值的吻合保持不变，斜率缺口如实显现并归因于亚畴协同效应。

![图4.21 器件双模型一致性](figs/Chapter04_local_21.png)

**图4.21** 器件双模型一致性。紫线为对实测数据回归的开关概率Sigmoid，红点为宏自旋LLG随机求解器 (每点200次蒙特卡洛，误差棒为Wilson 95%区间)，竖虚线为标定阈值；两模型判决阈值吻合到约$$0.4\,V_T$$ (约9 mV)，但LLG过渡斜率较实测Sigmoid更平缓——单宏自旋模型 (与Néel-Brown一致) 低估了C2C收窄后的实测斜率 ($$\eta_c$$缺口)，高压端的进一步偏离对应大过驱下的进动回切。
...
关键在于这一失调能否被读出链路吸收。以电流灵敏读出的跨阻$$R_\mathrm{TI}$$为桥，把毫伏失调折算到popcount域，有$$\sigma_\mathrm{pc}=\sigma_\mathrm{off}\cdot 2\,\mathrm{PC_{FS}}/V_\mathrm{in}$$，其中$$\mathrm{PC_{FS}}\approx 3\sqrt F$$为扇入$$F$$决定的满量程popcount，$$V_\mathrm{in}$$为比较器差分输入范围，跨阻取动态范围允许的最大增益。这给出一条协同设计准则：跨阻增益由扇入设定，并据此判定何时平凡比较器即足够。取扇入1024、$$V_\mathrm{in}=0.6\,\mathrm V$$，准则给出$$R_\mathrm{TI}=613\,\Omega$$；在sky130上以popcount正比的差分电流驱动该前端并扫描，提取失调在整条链路中映射到约2.5个popcount，落在精度曲线膝点之下，从晶体管级确认了在该扇入与输入范围下平凡比较器即足够 (图4.15)。因此对常见扇入与$$V_\mathrm{in}\ge0.5\,\mathrm V$$，平凡比较器即为帕累托最优；仅在低压、宽扇入、增益低于预算的列才需启用失调消除，可只在越过膝点的列触发单容自调零 (隐藏于地址译码、较双容方案省约15%面积、无时序代价[^sa_singlecap])，其余列直接用裸比较器，从而把磁存储读出中始终开启的失调消除[^sa_singlecap]改为按扇入与斜率条件触发；更高分辨的双StrongARM锁存可把基线失调再降约30%[^dsa][^dsa_meas]，可作为备选拓扑。读出端节省的校准由写端补偿：每列残余的系统性阈值偏移折叠进写数模转换器的逐列3至4位静态微调，其附加开关能量不足单次写能量的1%。把这一读出与近期设计对照，可见取舍差异：面向低隧穿磁阻存储的单容自调零灵敏放大器把失调降逾六成以保住约二倍阻比的小读窗[^sa_singlecap]，电荷舵双尾比较器以约$$0.3\,V_T$$的更低失调应对宽共模输入[^sa_doubletail]，二者均把失调视作须预先消除的负担；存内计算宏则多以列共享ADC摊销数字化、把比较器失调并入量化非理想[^cim_xnor_sram]。本文与之不同：先以斜率匹配把失调折算到$$V_T$$判决窗、再仅对越过膝点的列条件触发消除，于是名义工况下省去其面积与能量，并独立处理本任务特有的低阻写线IR。
...
**图4.15** 斜率匹配读出的失调预算与帕累托。(a) sky130 StrongARM输入折合失调 ($$0.39\,V_T$$) 与判决窗$$V_T$$的相对关系，失调预算以$$V_T$$为标尺。(b) 扇入1024、$$V_\mathrm{in}=0.5\,\mathrm V$$下四种失调消除方案的精度跌幅对剩余失调，除最低压宽扇入区外跌幅均在统计涨落内，平凡比较器位于帕累托前沿。(c) 五种读出比较器 (本文StrongARM、double-tail、DSA、电流采样、单电容自调零) 在同一sky130失调蒙特卡罗夹具下的输入折合失调 ($$\sigma_\mathrm{off}/V_T$$，误差棒为$$\pm1$$标准误)：四种失配受限拓扑同落于约$$0.39\,V_T$$且误差棒相互重叠，文献在更先进节点报告的拓扑间失调差异在此并不复现；单电容自调零实测降至$$0.167\,V_T$$ (约58%降幅，与文献28 nm报道的逾六成同量级)，但标称工况$$0.39\,V_T$$已在窗口预算内，故器件最少的单级StrongARM仍为帕累托最优，自调零留作越过膝点的列的条件触发选项。各备选电路与同流程分析见附录D。

写通路的电路级评估同样把器件级数值与外围开销分列。器件沟道的纯欧姆写能量为0.78 pJ；以sky130标准1.8 V CMOS反相器驱动写支路并扫描上拉管宽度，要把约0.9 V写电压施加到器件需约7 µm上拉管，此时器件吸收0.785 pJ而电源输出约1.6 pJ，端到端约为器件级的两倍，开销来自驱动导通电阻的分压；缩小驱动会欠驱导致远端写失败，放大驱动会过驱导致器件能量随平方升高，故高效的写需要一条受控的约0.9 V写电压轨，应并列报告器件级0.78 pJ与端到端约1.6 pJ两个数 (图4.16)。供电完整性方面，用Magic电阻提取在sky130上标定各层方块电阻，按真实列写线长度标度：列高$$N\le64$$时金属写线往返寄生电阻不足5%，$$N=256$$时在met2、1 µm线宽下达约128 Ω即16.5%[^te_ir]，误用局部互连层则达千欧量级；设计指导为写线走met2及以上、加宽或对高列分段。
...
**图4.16** 写通路能量与供电完整性。(a) 列写线往返金属寄生电阻比例随列高$$N$$增长 (met2、1 µm线宽)，金色带为约10–20%的裕度区。(b) sky130 1.8 V CMOS反相器输出平顶电压与驱动能量开销随上拉管宽度的变化。

![图4.17 IR感知逐行写预畸变](figs/Chapter04_local_17.png)

**图4.17** 沿高列的写通路设计与文献能力对照。(a) 无补偿时单元写电压随行号线性下降、远端约148 mV跌破标定$$V_\mathrm{th}$$ (红色阴影区写入失败)；金色三角为逐行寻址时列首施加的5位量化驱动码，紫色圆点为量化预畸变后单元实际所见电压，各行均保持在目标电压。(b) 在三个目标工作点 ($$P_\mathrm{target}=0.5/0.9/0.99$$) 下，无补偿写概率均沿列塌陷，预畸变把每行$$P_\mathrm{sw}$$拉平到各自目标 (虚线)；红色圆点为0.90目标下量化预畸变的逐行结果，与目标线重合。(c) 三种写数模转换器拓扑 (本文电压型电阻串、二进制加权电流舵、R-2R梯形) 在同一码扫描下送入写负载的积分非线性与单调性：仅电压型电阻串按构造单调 (积分非线性约0.48个最低有效位)，电流舵与R-2R分别因源漏裕度塌缩与进位处开关导通电阻扰动而失去单调性 (积分非线性约1.7与2.6个最低有效位)。与未在本地复现的更广文献的定性能力对照见附录D。
...
**图4.19** 三类操作的瞬态波形。(a) 写脉冲施加与瞬时开关概率 (含标定紧凑模型)；(b) StrongARM锁存再生，时钟沿后由预充态分裂为轨到轨判决 (10 mV差分输入)；(c) 列共享逐次逼近转换器的电荷再分配逐位逼近 (储备池读出，详见第五章)。(a)(b)为ngspice瞬态。

![图4.20 工作模式流水线与相位时序](figs/Chapter04_local_20.png)

**图4.20** 工作模式流水线。(a) 概率位推断：每样本为写、弛豫、读相位循环，相位使能波形$$\Phi_\mathrm{W}$$/$$\Phi_\mathrm{R}$$与各相位对齐，$$T$$次平均得期望。(b) 储备池处理 (第五章)：写入输入、低势垒自由演化、列共享逐次逼近转换器分时扫描各列；上方波形为写入使能、单节点随机电报态$$m(t)$$ (其驻留时间即被计算的记忆) 与逐列读出选通。两模式分时复用同一阵列、在时间上互斥。

上述外围模块最终落到存储单元的版图与集成结构上 (图4.22)。2T单元由写、读两只访问管与其金属引出构成：写访问管须通过约1.16 mA写电流、取约2.2 µm管宽，读访问管取最小宽度；单元版图以工艺套件的参数化单元脚本化生成，CMOS部分经官方设计规则集检查零违例[^drc_control]，寄生提取还原出两只晶体管的准确尺寸与约3 fF的单元内互连电容。磁隧道结柱与SOT沟道无开源工艺可承载，依混合CMOS/磁性工艺套件的既定做法[^hybrid_mpdk]以注记层黑盒表示：黑盒插于met2与met3之间，底电极焊盘与源线落在met2 (与写线走met2及以上的设计规则相衔接)，顶电极经met3读线引出；其平面几何以公开报道的器件外形为锚，即柱临界尺寸约80 nm、沟道宽200 nm、底电极间距200 nm[^hikstor_edl]，垂直堆叠细节无法从公开信息复现，仍按晶圆标定输入处理。实绘单元含字线、位线引出与独立体接触条，设计层包络约22.7 µm²，为4.6 µm²的阵列摊销估算给出上界，二者之差即单元级引出与共享体接触、邻接布线之间的口径差。SOT层材料的自旋霍尔角公开报道约为−0.3 (钨，4 nm)[^hikstor_edl][^beta_w]，第二章标定的器件级有效自旋霍尔效率显著低于该材料值，差距源于自旋透明度与电流分流等界面及几何因子，引用时两种口径不可混用。

[^drc_control]: 设计规则检查曾出现方法学假阴性：所用官方规则集把规则封装在前段、后段与网格三组开关之下，缺省调用不执行任何规则而恒报零违例；该问题经注入已知违例的阳性对照显形并修正，此后所有零违例结论都以阳性对照先行验证。

![图4.22 2T存储单元版图与集成结构](figs/Chapter04_local_22.png)

**图4.22** 2T存储单元的sky130版图与垂直集成结构。(a) 版图顶视：左为体接触条，中为写访问管 (管宽2.2 µm)，右为读访问管 (旋转放置、栅极引出朝下)，字线、位线与源线经met1/met2引出，SOT沟道 (橙) 与磁隧道结柱 (紫) 为注记层黑盒；(b) 剖面示意 (未按比例)：黑盒插于met2与met3之间，底电极与源线落met2、顶电极经met3读线引出，红、蓝箭头分别为写、读电流路径。黑盒层不可制造，CMOS部分经设计规则检查零违例。

器件结构参数组本身也可作自洽核验 (图4.23)。以第二章的界面各向异性、饱和磁化、自由层厚度与电学直径正演保持势垒 (含精确扁椭球退磁因子)，得约48.5 kT，与同平台记忆级器件公开报道的保持带59–64 kT同量级；有效各向异性是界面项与形状项之差，二者补偿约81%，界面各向异性每变化1%即令保持势垒移动约5.3%，这一近补偿放大既把与公开带的差距归结为百分之几的叠层参数精度，也定量解释了标定所得约7.7%的器件间热稳定因子离散仅需约1.4%的微观涨落。此处正演的是保持势垒，与写概率标定的开关律指数$$\Delta=4.91$$分属不同物理量；同一叠层要把保持势垒压到超顺磁工作点，需把电学直径缩到约17 nm或把界面各向异性修剪约17%——修剪路线将补偿度由约81%深化至约98%，幅度虽不小，但仍在退火与界面工程的常规调控范围内，故超顺磁变体更可能来自更深补偿的叠层设计而非大幅几何缩小。单元间磁串扰由自由层磁矩的偶极场界定：磁矩仅约$$2.3\times10^{-18}\,\mathrm{A\cdot m^2}$$，最坏情形 (近邻全同态、含点阵求和裕量) 下概率工作点的偏移在实绘单元间距处为$$10^{-6}$$量级，黑盒抽象在磁学上自洽；偏移达1%的临界间距约211 nm，仅无访问管的密集后段阵列才需将磁串扰纳入预算，较常规确定性存储约30 nm即可满足的同类间距规则[^mram_pitch]严苛约一个数量级——低势垒器件对杂散场的指数级敏感性把磁串扰间距要求推入亚微米，但仍远小于访问管决定的微米级单元间距。器件内固定层的静态杂散场由合成反铁磁叠层在设计上补偿，不属此处的单元间耦合。

![图4.23 器件结构自洽核验与磁串扰界](figs/Chapter04_local_23.png)

**图4.23** 器件结构参数组的自洽核验与阵列磁串扰界。(a) 由结构参数组正演的保持势垒随电学直径的变化：标定直径处约48.5 kT，与公开报道的记忆级保持带同量级；阴影带为$$K_i\pm1\%$$的敏感度包络 (近补偿放大)，金色虚线为界面各向异性修剪17%后的同一曲线——压到超顺磁开关律指数所在水平需缩径至约17 nm或修剪$$K_i$$约17%；(b) 最坏情形近邻概率偏移随阵列间距的变化：偏移1%的临界间距约211 nm (红色阴影为需预算磁串扰的无访问管密集后段区)，2T单元估算间距处偏移约$$10^{-5}$$、更宽的实绘单元间距下低至$$10^{-6}$$量级，较常规确定性存储约30 nm的同类间距规则 (灰色点划线) 严苛约一个数量级。
...
外围链条的最后一环，是把行为级与电路级在同一随机实例上对齐的端到端验证。架构把随机权重比特的产生留在数字域 (写时序按目标概率择极性)，据此本工作采用确定性重放式共仿：由同一随机源预抽整列伯努利态，同一组态分别送入行为级popcount判决与晶体管级读出链瞬态仿真，逐次比较判决——验证对象是位线压降、跨阻映射、灵敏放大器失调与再生、建立过程的联合效应，而非写入随机性本身。电路为64单元差分列，位线以提取板阻建模为分段梯形网络，终端为虚地跨阻级与sky130 StrongARM灵敏放大器；此处采用虚地出于架构必需：列的诺顿阻抗仅约百欧姆量级，无源电阻感测会同时压塌共模与popcount斜率，且列共模电流须由匹配参考列抵消、不得流经反馈电阻[^replay_iter]。600次重放 (灵敏放大器失调取0与9.2 mV两个变体) 给出三层结果 (图4.25)：灵敏放大器对模拟符号的保真度分别为0.998与0.947，失调变体的差错全部集中在判决边界附近的小裕量样本；行为级理想映射与电路之间的主要偏差是位线电阻引起的popcount斜率压缩 (拟合斜率约0.69，远端压降约为读半摆幅的四分之一)，对该偏差作一点阈值重标定后判决一致率由86%回升到92%，重标定后的残差分布通过Lilliefors正态性检验；同一网络的线性精确解与电路仿真在微伏量级吻合，确认压缩来自线阻物理而非仿真伪影。对设计的可执行含义：行为级默认的理想读出映射在窄金属位线与微米级间距下并不成立，须以更宽或更高层金属走位线、或在判决阈值中吸收斜率压缩；此前按单元电阻量级判定读通路压降可忽略，其正确的比较对象应为列诺顿阻抗；本验证据此把该判断修订为可量化的设计规则。用同一精确解把列高从16扫到256给出该规则的定量形式[^ir_nscan]：斜率匹配预算下，无补偿的阈值吸收在列高32以内成立 (斜率不低于0.90、判决无损)，64恰在边界外 (斜率0.69、对理想比较器的判决一致率95.5%)，128及以上斜率压缩造成的信噪比损失使一点重标定亦无法挽回。

[^ir_nscan]: 列高扫描以与ngspice在64单元处亚微伏吻合的精确线性求解器外推：$$N=16/32/64/128/256$$的拟合斜率为0.98/0.90/0.69/0.35/0.13，远端压降0.9/3.9/13.0/31.5/46.4 mV，对理想比较器的原始判决一致率100/100/95.3/79.7/59.8%，一点斜率重标定后为100/100/95.5/83.2/66.7%。与正文600次重放的86%口径不同：重放含灵敏放大器失调与再生动态，此处隔离位线梯形网络这一单一通道。

[^replay_iter]: 该读出结构经两次真实迭代才收敛：初版以无源电阻直连共模电压感测，被列诺顿阻抗 (约110 Ω对2450 Ω跨阻) 压塌至每popcount单位不足1 mV、低于放大器失调；改虚地后初版未抵消共模电流，跨阻级输出被拖至地电位以下、灵敏放大器输入对截止，其判决实为微伏级泄漏残余的符号——加入匹配参考列的共模抵消后放大器才满轨再生，此处数字均为修正后结果。
...
本章把全文主线下同一硬件单元在机器学习推断任务上的可达性能与边界这一问题，落到一条端到端硬件仿真流水线之上。其关键设计是让软件基线、硬件感知与全栈评估三档模式共享同一份潜参数检查点：三档既共用$\boldsymbol{\Theta}$张量，也借助硬二值STE技巧复用BN滑动统计，使训练所得检查点无需重新训练即可切换到真实硬件语义下加以评估。这一安排避免了硬件感知训练中常见的训练与推理语义不一致带来的偏差，使同一仿真器能够同时服务于训练循环、PPA估算与鲁棒性扫描。配套的CLT高斯化前向把训练阶段的$T$次显式Bernoulli抽样压缩为一次解析近似，使每步训练复杂度回到与确定性矩阵乘法相近的量级——这一步使PBNN由算法可行但训练昂贵，推进到训练代价与确定性BNN持平。

鲁棒性分析揭示了PBNN相对FP位置编码的结构性优势：FP编码中高位承担更大动态范围，MSB翻转会造成远大于低位翻转的误差；PBNN则将权重展开为$T$个独立Bernoulli样本，每个样本对输出的贡献幅度均受$2/T$约束，因此单点翻转的最坏影响由编码方式直接限制。该机制解释了PBNN在比特翻转扫描中的稳定性，也表明其可靠性收益并非依赖额外纠错电路。非理想性消融也给出了清晰的工程结论：影响精度的主要瓶颈是$V_\mathrm{th}$的绝对位置稳定性，而斜率漂移、循环噪声与back-hopping可分别通过BN、$T$步平均和工作点回退缓解。硬件优化因而可优先聚焦于DAC校准与温漂补偿。

跨架构能耗对比进一步说明，sMTJ-PBNN相对CMOS p-bit ASIC的优势来自Bernoulli发生这一基本操作的物理代价差异。与确定性STT-MRAM CIM相比，PBNN新增能耗主要集中在前向采样通路，反向传播与权重更新通路的代价基本保持同量级。PBNN由此以可量化的前向能耗开销换取比特翻转鲁棒性、采样数可调的精度-能耗折线以及原生不确定性量化能力，这一取舍可在系统层面显式评估。

从主线层面看，本章工作的根本意义在于与第三章构成对偶：第三章在伊辛求解任务上验证了同一物理阵列能够以可机理归因的优势承担组合优化，本章则在PBNN推断任务上验证了同一物理阵列能够以同量级能效与结构性鲁棒性承担机器学习。两类任务之间在器件层、阵列层与仿真后端层均无硬件分歧，区别只在于外围调度——这正是第一章所设想的、由同一阵列在不同时间窗口分别承担两类传统分离任务在工程层面的可证明形态。综合两章结果，全自旋三位一体架构与时域展开范式在任务、架构与非理想性三个维度上均得到了可复现的定量支持，下一章将把同一器件由瞬时采样源延伸为有状态的动力学节点，检验其承担时序处理任务的能力。
...
[^eot_athalye]: Athalye A, Carlini N, Wagner D. Obfuscated gradients give a false sense of security: circumventing defenses to adversarial examples. *Proceedings of the 35th International Conference on Machine Learning (ICML)*, 2018: 274–283. [arXiv:1802.00420](https://arxiv.org/abs/1802.00420)

[^eot_tramer]: Tramèr F, Carlini N, Brendel W, Madry A. On adaptive attacks to adversarial example defenses. *Advances in Neural Information Processing Systems (NeurIPS)*, 2020, 33: 1633–1645. [arXiv:2002.08347](https://arxiv.org/abs/2002.08347)
...
[^hikstor_edl]: Yang W, Ji Z, Gao Y, Zhou K, Guo Q, Zeng D, Wang S, Wang M, Shen L, Chen G, Sun Y, Liu E, He S. Achieving high yield of perpendicular SOT-MTJ manufactured on 300 mm wafers. *IEEE Electron Device Letters*, 2024, 45(11): 2094–2097. [doi:10.1109/LED.2024.3454609](https://doi.org/10.1109/LED.2024.3454609)

[^beta_w]: Pai C-F, Liu L, Li Y, Tseng H W, Ralph D C, Buhrman R A. Spin transfer torque devices utilizing the giant spin Hall effect of tungsten. *Applied Physics Letters*, 2012, 101(12): 122404. [doi:10.1063/1.4753947](https://doi.org/10.1063/1.4753947)
...
[^shevtsova2010]: Shevtsova I G. An improvement of convergence rate estimates in the Lyapunov theorem. *Doklady Mathematics*, 2010, 82(3): 862–864. [doi:10.1134/S1064562410060062](https://doi.org/10.1134/S1064562410060062)
